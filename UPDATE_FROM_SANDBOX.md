...
## Premium-consistency audit + completion round (2026-09-23)
- **Honest audit ran first** — the checklist was NOT fully done before this round. Gaps found and closed in place:
- **Component system COMPLETE** — `lib/core/widgets/ui.dart` now covers all 12 requested shapes: PageHeader · SectionHeader · SettingsRow · MhuriField (form field + currency input: decimal keyboard + symbol prefix) · PrimaryButton (primary) · SecondaryButton (secondary + danger=destructive outline) · confirmDialog (dialogs) · showMhuriSheet + SheetHeader (bottom sheets) · EmptyHint/EmptyState (empty) · ErrorNotice (error notices) · MemberAvatar (avatars) · TxTile (transaction rows) · Skeleton (loading/sync). Adoptions this round: budgets header + envelope sheet (SheetHeader + showMhuriSheet + MhuriField), member photo buttons (SecondaryButton), member tile avatar (MemberAvatar), envelope detail labels (MhuriField), sync rows (SettingsRow), setup error box (ErrorNotice). USD/ZiG segment labels and `you@example.com` stay literal by design (currency codes / email format are not translatable content).
- **"Replace prototype content" FINISHED**: last embedded English strings extracted (~20): create/join dialogs (Cancel/Create/Join via Material + btnCreate/btnJoin), kids PIN dialog (hint + "Kids Mode PIN updated ✓"), 'e.g. The Taylor Family' family-name hint (the literal "hardcoded family name" item), budgets transfer From/To/Why?, lists Item/Qty + 'All (n)' chip (parameterized), 4 setup error literals, invite '…' placeholder error → real message. l10n +17 ×6 → **558** (generated ×7, full parity).
- **No technical error messages**: both `AvatarException e.message` leaks (members, setup) now show localized avatarError. All server errors were already mapped to friendly localized text (makeOwnerFailed, inviteTooMany, syncLastError…).
